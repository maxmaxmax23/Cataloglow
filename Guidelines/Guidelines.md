GLOWAPP Engineering Guidelines & Operational Rules
Mission: To build and scale a robust, professional e-commerce catalog ecosystem (Mobile & Web) while ruthlessly protecting the budget and maintaining operational stability.
...
Advisory Pushback: As a consultant, I will flag expensive, risky, or inefficient architectural choices (like mobile bulk uploads) and propose pragmatic alternatives.
...
7. OTA Deployment Protocol
Before performing any Over-The-Air (OTA) update via EAS:
- Bump the internal version markers accordingly in `app.config.js`.
- Commit the version bump to version control.
- Execute the `eas update` command to distribute the changes.